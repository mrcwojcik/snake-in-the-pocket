    ...
    <div id="adminContent">
        ...
        <div id="panel">
            <c:if test="${fn:length(accounts) > 0}">
                ...
                <br/>
                <h3>Kilka kryteriów na raz:</h3>
                <select id="accountMixReport" class="reportForm">
                    <c:forEach items="${accounts}" var="account">
                        <option value="${account.id}">${account.accountName}</option>
                    </c:forEach>
                </select>
                <select id="payerMixReport" class="reportForm">
                    <c:forEach items="${payers}" var="payer">
                        <option value="${payer.id}">${payer.name}</option>
                    </c:forEach>
                </select>
                <select id="categoryMixReport" class="reportForm">
                    <c:forEach items="${categories}" var="category">
                        <option value="${category.id}">${category.categoryName}</option>
                    </c:forEach>
                </select>
                <button id="mixReportBtn" class="reportBtn">Filtruj</button>
                <p>=====================================================</p><br/>
                <c:if test="${!empty reportBills}">
                    <h2>WYNIKI</h2>
                    Wydano: ${valueOnMinus} zł<br/>
                    Przybyło: ${valueOnPlus} zł<br/>
                    Difference: <c:set var="total" value="${valueOnPlus - valueOnMinus}"/><c:out
                        value="${total}"/> zł<br/>
                    <p>=================</p><br/>
                ...
                <c:if test="${empty reportBills}">
                    <b style="color: red">Nie znaleziono rachunków spełniających kryteria.<b/>
                </c:if>
            </c:if>
            <c:if test="${fn:length(accounts) == 0}">
                Nie możesz sprawdzić żadnych raportów, bo nie dodałeś jeszcze konta.<br/>
